...
def train_and_compare_models(early_fusion=True, late_fusion=True, num_epochs=5):
    ...
    # Initialize models
    if late_fusion:
        models['late_fusion'] = HatefulMemesModel().to(device)
    
    if early_fusion:
        models['early_fusion'] = EarlyFusionHatefulMemesModel(use_cross_modal_attention=True).to(device)
    
    # Setup for each model
    for model_name, model in models.items():
        # Initialize evaluator
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        log_dir = f"/kaggle/working/runs/hateful_memes_{model_name}_{timestamp}"
        evaluators[model_name] = KaggleHatefulMemesEvaluator(model, device, log_dir=log_dir)
        ...
        if model_name == 'late_fusion':
            ...
            for name, param in model.named_parameters():
                if param.requires_grad:
                    ...
                    else:  # fusion_classifier or attention parameters
                        fusion_params.append(param)
            
            optimizers[model_name] = torch.optim.AdamW([
                {'params': image_params, 'lr': 1e-4},
                {'params': text_params, 'lr': 1e-5},
                {'params': fusion_params, 'lr': 1e-4}
            ], weight_decay=0.001)
        
        # Learning rate scheduler
        schedulers[model_name] = torch.optim.lr_scheduler.StepLR(
            optimizers[model_name], step_size=2, gamma=0.5
        )
        
        # Loss function with class weights
        pos_weight = torch.tensor([2.0]).to(device)
        criterions[model_name] = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        
        # Initialize tracking variables
        training_times[model_name] = 0
        inference_times[model_name] = 0
    ...
    for epoch in range(num_epochs):
        ...
        for model_name, model in models.items():
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                with autocast('cuda'):
                    outputs = model(images, input_ids, attention_mask)
                    loss = criterions[model_name](outputs.squeeze(), labels)
                
                optimizers[model_name].zero_grad()
                scaler = GradScaler()
                scaler.scale(loss).backward()
                scaler.unscale_(optimizers[model_name])
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=0.1)
                scaler.step(optimizers[model_name])
                scaler.update()
                
                total_loss += loss.item()
                
                # Log batch loss to TensorBoard
                evaluators[model_name].writer.add_scalar('Batch/train_loss', loss.item(), 
            ...
            training_times[model_name] += epoch_time
            
            avg_train_loss = total_loss / len(train_loader)
            print(f'Average Training Loss: {avg_train_loss:.4f}, Time: {epoch_time:.2f}s')
            
            # Log average training loss
            evaluators[model_name].writer.add_scalar('Epoch/train_loss', avg_train_loss, epoch)
            
            # Validation phase
            print(f"Validating {model_name} model...")
            start_time = time.time()
            val_metrics = evaluators[model_name].evaluate(val_loader, epoch, mode='val')
            epoch_inference_time = time.time() - start_time
            inference_times[model_name] += epoch_inference_time
            ...
            evaluators[model_name].log_sample_predictions(batch, outputs, epoch, mode='val')
            
            # Analyze model performance (every 2 epochs to save time)
            if epoch % 2 == 0:
                evaluators[model_name].analyze_model_performance(val_loader, epoch, mode='val')
    ...
    if len(models) > 1:
        ...
        for model_name in models:
            print(f"{model_name} - Training time: {training_times[model_name]:.2f}s, "
                  f"Inference time: {inference_times[model_name]:.2f}s")
        
        # Use the evaluator to compare models
        first_evaluator = list(evaluators.values())[0]
        first_evaluator.compare_models(models, val_loader, mode='val')
        ...
        # Generate model results dictionary
        
        for model_name, evaluator in evaluators.items():
            model_results[model_name] = {
                'auroc': evaluator.best_auroc,
                'f1': evaluator.best_f1,
                'training_time': training_times[model_name],
                'inference_time': inference_times[model_name]
            }
        
        # Generate the analysis report
        first_evaluator.generate_analysis_report(model_results, dataset_info)
    ...
    return models, evaluators, model_results

# Run the training and comparison
if __name__ == "__main__":
    models, evaluators, results = train_and_compare_models(
        early_fusion=True, 
        late_fusion=False,
        num_epochs=1
    )